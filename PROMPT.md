...
## Update: subscription usage gauges

A later update moved 🪙 tokens and ⏱️ elapsed up to line 1, and added Pro/Max
subscription usage gauges (⚡ 5-hour and 📅 7-day, each with a `↻` reset
countdown and a `⚠️` warning at ≥ 90%) to line 2. The gauges read the
`rate_limits` block, which is absent until the first API response and on plans
without subscription limits, so the script omits them when the data isn't
present. The host segment is kept green, consistent with the earlier tweak.

> Set up a custom two-line Claude Code status line for me. Create the file
> `~/.claude/statusline.sh` with exactly this content:
>
> ```bash
> #!/bin/bash
> # Claude Code status line.
...
> fi
>
> LINE1="${CYAN}[$MODEL]${RESET} ${MAGENTA}🖥️ ${HOST}${RESET} 📁 ${DIR##*/}${BRANCH} | 🪙 ${TOKENS} | ⏱️ ${MINS}m ${SECS}s"
...
> 2. Confirm jq is installed (`command -v jq`); if not, install it (`apt-get install -y jq` or your distro's equivalent).
> 3. Merge this into `~/.claude/settings.json` as a top-level key, preserving everything else already in the file:
>    ```json
>    "statusLine": {
>      "type": "command",
>      "command": "~/.claude/statusline.sh",
